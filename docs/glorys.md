...
所以**网络流量远大于落盘体积**。dry-run 返回的 `data_transfer_size` 就是真实流量。
...
**同一块内的年份必须合并成一次请求**，否则同一块会被反复拉。脚本自动按块分组，
CLI 上感觉不到。磁盘装不下整块时，分几次调用、每次只给一个年份即可——
块是按请求时段算的，给单年就只拉单年。
...
- **磁盘峰值**。多年块要先落一个完整的中间文件再切分，峰值约是最终体积的 2 倍。
